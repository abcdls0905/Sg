...
public class GameTimer : MonoBehaviour {
    ...
    public static GameTimer StartRealTimer(float time, float repeat, bool once, bool destroyOnLoad, string name, EventHandler receiver, string tname = "timer")
    {
        var go = new GameObject(tname);
        var com = go.AddComponent(typeof(GameTimer)) as GameTimer;
        com.timeLeft_ = time;
    ...
    private void Update() {
        if (isRealTime_)
            timeLeft_ -= Time.unscaledDeltaTime;
        else
