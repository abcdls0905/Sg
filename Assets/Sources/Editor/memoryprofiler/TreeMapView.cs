    ...
    public class TreeMapView
    {
        ...
        private void RefreshCachedRects(bool fullRefresh)
        {
            ...
            if (fullRefresh)
            {
                ...
                Rect[] groupRects = Treemap.Utility.GetTreemapRects(groupTotalValues, space);
            ...
            {
                Rect[] rects = Treemap.Utility.GetTreemapRects(_selectedGroup.memorySizes, _selectedGroup._position);
        ...
        private void RenderLabels()
        {
            ...
            foreach (Group group in _groups.Values)
            {
                if (Treemap.Utility.IsInside(group._position, _ZoomArea.shownArea))
                {
        ...
        private void RenderGroupItems(Group group)
        {
            ...
            foreach (Item item in group._items)
            {
                if (Treemap.Utility.IsInside(item._position, _ZoomArea.shownArea))
                {
